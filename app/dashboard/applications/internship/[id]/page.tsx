...
export default async function InternshipApplicationPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="container max-w-4xl py-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div>
           <h1 className="text-2xl font-bold mb-2">Internship Application</h1>
           <div className="flex items-center gap-2 mb-6">
             <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                app.status === 'Accepted' || app.status === 'Approved' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' :
                app.status === 'Rejected' || app.status === 'Dropped' ? 'bg-red-100 text-red-800 border-red-200' :
           ...
           <div className="bg-card rounded-lg border p-6 space-y-4">
             <div>
               ...
               <div className="font-medium">{app.institution}</div>
             </div>
             <div>
             ...
             <div>
               ...
               <div className="font-medium">{app.duration || "-"}</div>
             </div>
           </div>
        </div>
