...
async function updateRole(formData: FormData) {
  ...
  let permissionConnect: { id: string }[] = []
  if (permissionsJson) {
...
async function createRole(formData: FormData) {
  "use server"
  const { role: currentUserRole } = await requireRole(["Admin", "SYSTEM_ADMIN", "SUPER_ADMIN"], "manage_users") // High-level only
...
async function deleteRole(formData: FormData) {
  "use server"
  const { role: currentUserRole } = await requireRole(["Admin", "SYSTEM_ADMIN", "SUPER_ADMIN"], "manage_users")
